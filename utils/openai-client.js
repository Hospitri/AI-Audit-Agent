...
const SYSTEM_PROMPT = [
    ...
    '- HARD CAP: no score (overall or any category numeric) may exceed 8.0. If your internal estimate is higher, OUTPUT 8.0.',
    '- Target distribution: median ≈ 5.5; most listings land 4.5–6.5; 7.5–8.0 is exceptional and rare.',
    '- Penalize missing, incomplete, generic, or ambiguous evidence. If unsure, assume below average (4–5).',
    '',
    'Calibration anchors:',
    '3.0 = poor (multiple gaps; unclear value).',
    '5.5 = average (complete but generic; several improvable areas).',
    '7.0 = strong (clear, specific, few weaknesses).',
    '8.0 = exceptional (rare; outstanding in most categories).',
    '',
    'Category caps/heuristics (examples, not exhaustive):',
    '- title: generic/keyword-stuffed → cap 5.5; distinctive, value-driven → up to 8.0.',
    '- description: <250–300 words or generic → cap 6.0.',
    '- images: <10 usable, low diversity, obvious stock → cap 6.0; excellent variety/quality → up to 8.0.',
    '- amenities: missing key items or unclear → cap 6.0.',
    '- reviews: few/old/absent → cap 5.0; excellent volume+recency → up to 8.0.',
    '- pricing: unclear fees or value narrative → cap 6.0.',
    '- policies_fees: hidden/unclear → cap 5.5.',
    '- response_speed: no evidence → 4–5; clear ≤2h → up to 7–8.',
    '',
    ...
    'Return overall_score as 0.0–8.0 (1 decimal). Never exceed 8.0.',
    '',
    ...
    const messages = [
        { role: 'system', content: SYSTEM_PROMPT },
        {
            ...
            content: [
                ...
                `{
                    "overall_score": number (0..8, 1 decimal),
                    "category_breakdown": {
                        "title": {"numeric": int (0..8), "grade": "A|B|C|D", "what_works": string<=60, "what_to_improve": string<=60},
                        "description": {...},
                        "images": {...},
                        "amenities": {...},
                        "reviews": {...},
